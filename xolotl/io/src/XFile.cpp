	...
	int timeStep, double time, double previousTime, double deltaTime) const
{
	...
	if (groupExist) {
		// Get the group
		tsGroup = getTimestepGroup(ctrlStep, loop, timeStep);
...
std::unique_ptr<XFile::TimestepGroup>
XFile::ConcentrationGroup::getTimestepGroup(
	int controlStep, int loop, int timeStep) const
{
	std::unique_ptr<XFile::TimestepGroup> tsGroup;
	...
	try {
		...
		tsGroup = std::make_unique<XFile::TimestepGroup>(
			*this, controlStep, loop, timeStep);
	}
